...
@Component({
  selector: 'app-contrato-terminal-destino',
  templateUrl: './contrato-terminal.component.html',
  styleUrls: ['./contrato-terminal.component.css']
})

export class ContratoTerminalComponent {
  ...
  editDialog(item: Array<any>, url: string, table: string): void {
    const dialogRef = this.dialog.open(ContratoTerminalFormDialogComponent, {
      data: {
        itemsData: item,
        url: url,
        query: table
